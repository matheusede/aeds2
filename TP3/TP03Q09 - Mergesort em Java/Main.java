...
public class Main{
    ...
    public static void matricula(long time, int contador){ 
        ...
        try {
            File file = new File("matrículaMergesort.txt");
        ...
        try {
        FileWriter input = new FileWriter("matrículaMergesort.txt");
    ...
    public static void merge( Serie[] arrayDeSeries, Serie[] l, Serie[] r, int left, int right) {
        int i = 0, j = 0, k = 0;
        while (i < left && j < right) {
            if ( (l[i].getEpisodes() < r[j].getEpisodes()) || l[i].getEpisodes() == r[j].getEpisodes() && l[i].getName().compareToIgnoreCase(r[j].getName()) < 0) {
                arrayDeSeries[k++] = l[i++];
            }
            else {
                arrayDeSeries[k++] = r[j++];
    ...
    public static void mergeSort(Serie[] arrayDeSeries, int inputCounter) {
        if (inputCounter < 2) {
            return;
        }
        int mid = inputCounter / 2;
        Serie[] esquerda = new Serie[mid];
        Serie[] direita = new Serie[inputCounter - mid];
    
        for (int i = 0; i < mid; i++) {
            esquerda[i] = arrayDeSeries[i];
        }
        for (int i = mid; i < inputCounter; i++) {
            direita[i - mid] = arrayDeSeries[i];
        }
        mergeSort(esquerda, mid);
        mergeSort(direita, inputCounter - mid);
    
        merge(arrayDeSeries, esquerda, direita, mid, inputCounter - mid);
    }
	...
	public static void main(String args[]){
       ...
       mergeSort(arrayDeSeries, inputCounter);
