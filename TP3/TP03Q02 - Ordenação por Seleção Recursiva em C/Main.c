...
//Função que realiza  a ordenação por seleção
int selecao(int j, Serie arrayDeSeries[], int tamanho, int i){
    if(j == tamanho)
        return 0;
    else{
        if(strcmp(arrayDeSeries[i].pais, arrayDeSeries[j].pais) > 0){
            Serie aux = arrayDeSeries[i];
            arrayDeSeries[i] = arrayDeSeries[j];
            arrayDeSeries[j] = aux;
        }
        if(strcmp(arrayDeSeries[i].pais, arrayDeSeries[j].pais) == 0){
            if(strcmp(arrayDeSeries[i].nome, arrayDeSeries[j].nome) > 0){
                Serie aux = arrayDeSeries[i];
                arrayDeSeries[i] = arrayDeSeries [j];
                arrayDeSeries[j] = aux;
            }
    ...
    return selecao(++j, arrayDeSeries, tamanho, i);
}

int chamaSelecao(int i, int tamanho, Serie arrayDeSeries[]){
    if(i == tamanho)
        return 0;
    selecao(0, arrayDeSeries, tamanho, i);
    return chamaSelecao(++i, tamanho, arrayDeSeries);
}

int printer(Serie arrayDeSeries[], int size){
    if(size == -1){ return 0;}
    else
    {
        print_serie(&arrayDeSeries[size]);
        return printer(arrayDeSeries, --size);
    }
}
...
int main() {
    ...
    chamaSelecao(0, inputCounter, arrayDeSeries);

    printer(arrayDeSeries, inputCounter -1);

    clock_t end = clock();
    tempoDeExecucao += (double)(end - start) / CLOCKS_PER_SEC;


    matricula(numMatricula, contador, tempoDeExecucao);
