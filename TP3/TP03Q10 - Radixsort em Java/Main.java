...
class Serie{
    ...
    int episodes){
        ...
        this.doidao = this.episodes*1000 + this.seasons; 
    }
...
public class Main{
    ...
    public static void matricula(long time, int contador){ 
        ...
        try {
            File file = new File("matrículaRadixsort.txt");
        ...
        try {
        FileWriter input = new FileWriter("matrículaRadixsort.txt");
    ...
    public static void countSort(Serie arraSeries[], int n, int exp)
    {
        ...
        for (i = 0; i < n; i++)
            count[(arraSeries[i].getDoidao() / exp) % 10]++;
 
        for (i = 1; i < 10; i++)
            count[i] += count[i - 1];
    
        for (i = n - 1; i >= 0; i--) {
            output[count[(arraSeries[i].getDoidao() / exp) % 10] - 1] = arraSeries[i];
            count[(arraSeries[i].getDoidao() / exp) % 10]--;
        }
 
        for (i = 0; i < n; i++)
            arraSeries[i] = output[i];
    }
    
    public static void radixsort(Serie arrayDeSeries[], int n, int m)
    {
        for (int exp = 1; m / exp > 0; exp *= 10)
            countSort(arrayDeSeries, n, exp);

        for(int i = 0; i < n - 1; i++){
            if(arrayDeSeries[i].getDoidao() == arrayDeSeries[i + 1].getDoidao() && arrayDeSeries[i].getName().compareTo(arrayDeSeries[i + 1].getName()) > 0){
                Serie aux = arrayDeSeries[i];
                arrayDeSeries[i] = arrayDeSeries[i + 1];
                arrayDeSeries[i + 1] = aux;

            }

            if(arrayDeSeries[i].getDoidao() == arrayDeSeries[i + 1].getDoidao() && arrayDeSeries[i + 1].getName().compareTo(arrayDeSeries[i].getName()) < 0){
                Serie aux = arrayDeSeries[i];
                arrayDeSeries[i] = arrayDeSeries[i + 1];
                arrayDeSeries[i + 1] = aux;
	...
	public static void main(String args[]){
        ...
        int maior = arrayDeSeries[0].getDoidao();

        for(i = 0; i < inputCounter; i++){
            if( arrayDeSeries[i].getDoidao() > maior){
                maior = arrayDeSeries[i].getDoidao();
            }
        }

        radixsort(arrayDeSeries, inputCounter, maior);
