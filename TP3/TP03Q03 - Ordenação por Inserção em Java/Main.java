...
public class Main{
    ...
    public static void matricula(long time, int contador){ 
        ...
        try {
            File file = new File("matriulaInsercao.txt");
        ...
        try {
        FileWriter input = new FileWriter("matriulaInsercao.txt");
	...
	public static void main(String args[]){
        ...
        //Odernação por inserção
        for(i = 1; i < inputCounter; i++){
            Serie tmp = arrayDeSeries[i];
            int j = i - 1;
            while ( ((j >= 0) && (arrayDeSeries[j].getLanguage().trim()).compareTo(tmp.getLanguage().trim()) > 0) ||(j >= 0) && 
            ((arrayDeSeries[j].getLanguage().trim()).compareTo(tmp.getLanguage().trim()) == 0 && arrayDeSeries[j].getName().trim().compareTo(tmp.getName().trim()) > 0)){
                arrayDeSeries[j + 1] = arrayDeSeries[j];
                j--;
                contaodrDeRepeticoes ++;

            }
            arrayDeSeries[j + 1] = tmp;
            contaodrDeRepeticoes ++;
        }
