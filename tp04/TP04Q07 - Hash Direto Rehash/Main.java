...
class TebalaHash {
	...
	public TebalaHash (int num){
        this.num = num;
        this.tabela = new Serie [this.num];
        for(int i = 0; i < num; i++){
            tabela[i] = NULO;
        }
    }
    public int resto(String elemento){
        int ascii =0;
    ...
    public boolean inserir (Serie elemento){
        ...
        if(elemento != NULO){
            int pos = resto(elemento.getName());
            if(tabela[pos] == NULO){
                tabela[pos] = elemento;
                resp = true;
...
class Main {
    ...
    public static void main (String []args){  
        ...
        TebalaHash hash = new TebalaHash();
